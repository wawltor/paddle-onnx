...
class TestBatchNormOp(unittest.TestCase):
    def ref_forward_backward(self, x, scale, bias, mean, variance,
                             epsilon, momentum, shape):
        # run forward
    ...
    def setUp(self):
        ...
        y = _reference_testing(x_val, scale_val, bias_val, mean, variance, epsilon).astype(np.float32)
    ...
    def eval_onnx_node(self):
        ...
        outputs = [
            paddle_variable_to_onnx_tensor("Y", self.block)
        ]
